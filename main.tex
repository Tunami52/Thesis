...
\begin{abstract}
  In 1972 Hajnal and Kertész published a paper proving that the Axiom of Choice is equivalent to the assertion that a groupoid structure exists every sets.
  ...
  This culminates in a lemma by Hartogs (1915) in which it is shown, without the Axiom of Choice, that for every set there exists an ordinal with cardinality greater than that of the set exists.
  A second part then gives an introduction to model theory with the aim to give a proof for the Löwenheim-Skolem Theorem.
  This states that every theory with an infinite model also has infinite models of any given power.
  The final part of the thesis then examines the proof by Hajnal and Kertész in detail.
\end{abstract}
