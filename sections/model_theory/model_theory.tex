...
We utilize set theoretic notation in the definition of languages, however we do not consider the set inclusion symbol $\in$ to be to be part of first-order logic here.
...
\begin{definition}[Formula of a Set]\cite[Definition 1.1.5]{Mar02}
    ...
    \end{enumerate}
    A \textit{formula} $\phi$ in $\mathcal{L}$ is an any combination of atomic formulas with logical connectives 
...
A language is purely \textit{syntactical}, a given sentences in a language has not assigned meaning on its own.
...
Having a relation be true or false is a \textit{semantical} property, this is where the namesake models come in.
