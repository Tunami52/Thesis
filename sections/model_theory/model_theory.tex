...
The following two theorems are given in the Chang and Keisler book in order to show that the model satisfaction relation is well defined.
I am somewhat unsure whether to include the proofs, since as the book puts it they are ``tedious but straightforward''.
On the other hand, this section barely has any interesting theorems, so including the proofs here would maybe add some necessary substance?
Then again the important theory needed in this chapter is in Section 3.4.

Note also that I will include some more examples of different models in the subsection 3.3.1 on theories of models.
There are plenty in both \cite{Cha90} and \cite{Mar02} and I will of course come up with some myself.
...
\begin{theorem}\cite[Proposition 1.3.16]{Cha90}
    ...
    \begin{enumerate}
        \item Let $t(v_0,\ldots,\, v_p)$ be a term dependent on the variables $v_0,\ldots,\, v_p$ and let $\bar{x} = \left<x_0,\ldots,\, x_q\right>$ and $\bar{y} = \left<y_0,\ldots,\, y_r\right>$ be sequences in $M$,
        such that $p \leq q$, $p \leq r$ and $x_i = y_i$ whenever $v_i$ is a variable of $t$.
        Then $$t(\bar{x}) = t(\bar{y}).$$
        \item Let $\phi$ be a formula in $\mathcal{L}$ with free and bound variables among $v_0,\ldots,\, v_p$ and let $\bar{x} = \left<x_0,\ldots,\, x_q\right>$ and $\bar{y} = \left<y_0,\ldots,\, y_r\right>$ be sequences in $M$,
        such that $p \leq q$, $p \leq r$ and $x_i = y_i$ whenever $v_i$ is a variable of $t$.
        Then $$\mathcal{M} \models \phi(\bar{x}) \quad \text{if and only if} \quad \mathcal{M} \models \phi(\bar{y}).$$
    \end{enumerate}
\end{theorem}
...
\begin{theorem}\cite[Proposition 1.3.16]{Cha90}
    ...
    such that none of the variables of $t$ occur bound in $\phi$.

    Now let $\bar{x} = \left<x_0,\ldots,\, x_p\right>$ be a sequence in $M$ bad let $\phi(t_0,\ldots,\, t_p)$ be the formula in $\mathcal{L}$,
