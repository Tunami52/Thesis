...
\begin{proof}\cite[Theorem 2]{Jec78}
...
\begin{definition}[Limit Ordinal]\cite[Exercise 2.3]{Jec78}
    We call an ordinal $\alpha$ a \textit{successor ordinal} if it is not a limit ordinal.
    Then $\alpha = \bigcup \alpha$ and $$\beta < \alpha \implies \beta + 1 < \alpha.$$
    The latter follows since $\beta + 1 \not< \alpha$ would imply that both $\beta \subset \alpha$ and $\beta \subset \beta + 1$ hold.
    But since $\alpha$ is not a successor ordinal by definition we have $\alpha \neq \beta + 1$, hence $\beta + 1 \in \alpha$ by contradiction.
\end{definition}
...
\begin{proof}\label{hartogs-lemma}\cite[Lemma]{Haj72}
