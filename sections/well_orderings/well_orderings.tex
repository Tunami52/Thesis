...
This means that any countable and densely ordered set has order type $\eta$ \cite[Theorem 11]{Jec78}.
...
\begin{definition}\cite[p.14]{Jec78}
    ...
    We say $\alpha < \beta$ if and only if $\alpha \in \beta$.\footnote{       
        Ordinals are sometimes also defined by being the equivalence class of all well-ordered sets which are order-isomorphic to each other.
        However, then an ordinal can not be a member of the set it describes, making it a proper class and not a set, which in turn is not suited to our axiomatic treatment of set theory. 
    }
\end{definition}
...
\begin{theorem}[Transfinite Induction]\cite[Theorem 3]{Jec78}
    ...
    \end{enumerate}
    Then $\mathbf{C}$ is the class of all ordinals.\footnote{
        Note that $\mathbf{C}$ must indeed be a proper class:
        if $\mathbf{C}$ was a set, then by definition it would also have to be an ordinal.
        By the last criteria of the theorem this would make $\mathbf{C}$ a member of itself; a contradiction by the \hyperref[ZF9]{Axiom of Foundation}.
    }
\end{theorem}
...
\begin{proof}
    ...
    \begin{equation}\label{hartogs-lemma-ordinal}
        \alpha = \bigcup \left\{\text{type}\left(X, R\right) + 1 \,\vert\, X \subseteq A, R \subseteq A \times A \wedge R\ \text{well-orders}\ X\right\}.
    \end{equation}
