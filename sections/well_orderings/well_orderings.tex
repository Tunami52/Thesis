...
\begin{example}\cite[Exercise 2.3]{Ros82}
    ...
    We use the fact that both $\mathbb{Q}$ and $\mathbb{Q} \setminus 0$ are countable.
    As such we can define 
    \begin{align*}
        & x_1,\, x_2,\ldots,\, x_i,\ldots \\
        \text{and} \quad & y_1,\, y_2,\ldots,\, y_i,\ldots
    \end{align*}
    as the sequences which enumerate $\mathbb{Q}$ and $\mathbb{Q} \setminus 0$ respectively.
    ...
    for $\mathbb{Q}$ and $1,\, -1,\, \frac{1}{2},\ldots$ for $\mathbb{Q} \setminus 0$.

    We now define a function $f: \mathbb{Q} \to \mathbb{Q} \setminus 0$ inductively:
    \begin{enumerate}
    ...
    Left to show now is that $f$ is also surjective.

    Let $y_i$ be some element in the sequence enumerating $\mathbb{Q} \setminus 0$.
    We now show that $y_i$ has a preimage in $f$.
    Say that $f$ has been defined up to $x_n$, then there are three cases.
    We must have that either $$y_i < f(x_j)\quad \text{or} \quad f(x_j) < y_i$$ for all $j \leq k$ or $$f(x_j) < y_i < f(x_k)$$ for some $h,\, k \leq n$.
    ...
    Now assume that $f(x_j) < y_i < f(x_k)$ for some $h,\, k \leq n$.
    ...
    As such every element $y_i$ in the sequence enumerating $\mathbb{Q} \setminus 0$ has a preimage in $f$.
    Because $f$ is also surjective, this means that $f$ is in fact a bijection.
    Hence we have defined an order isomorphism between $\mathbb{Q}$ and $\mathbb{Q} \setminus 0$.
\end{example}
